        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <div
                        class="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-lg min-h-[90px] text-left flex flex-col justify-start">
            ...
            <div class="flex justify-center gap-4 pt-4 border-t">
                <form action="{{ route('clock.in', $overtime->id) }}" method="POST" class="w-full sm:w-auto">
                    @csrf
                    <button
                        class="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-medium px-8 py-3 rounded-lg shadow transition">
                        Clock In
                    </button>
                </form>
                <form action="{{ route('clock.out', $overtime->id) }}" method="POST" class="w-full sm:w-auto">
                    @csrf
                    <button
                        class="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white font-medium px-5 py-3 rounded-lg shadow transition">
                        Clock Out
                    </button>
                </form>
            </div>

            {{-- Clock Details (Only show if clock-in exists) --}}
            @if ($overtime->clocks->whereNotNull('clock_in')->count() > 0)
                @include('overtime.clock-details')
            @endif

            <div class="mt-8 text-center">
            <a href="{{ route('overtime.index', ['employee_id' => $overtime->staff_id]) }}"
                class="inline-flex items-center justify-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow transition">
