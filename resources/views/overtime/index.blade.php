        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-4">
            <form method="GET" action="{{ route('overtime.index') }}"
                ...
                <div>
                    ...
                    <select name="status" class="mt-1 px-3 py-2 border w-full rounded-lg border-gray-300 shadow-sm text-sm">
                        <option value="">All</option>
                ...
                <div class="flex space-x-2 w-full">
                    <button type="submit" class="flex-1 px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-sm text-sm md:text-base">
                        Filter
                    </button>

                    <a href="{{ route('overtime.index') }}" class="flex-1 text-center px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg shadow-sm text-sm md:text-base">
                        Reset
                    </a>
                </div>
            </form>
                ...
                <div class="flex justify-between">
                    ...
                    <div>
                        @if ($req->status === 'pending')
                            <span class="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded-full">Pending</span>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($requests as $req)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-sm whitespace-nowrap">{{ $req->branch->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-sm whitespace-nowrap">{{ \Carbon\Carbon::parse($req->date)->format('d M Y') }}</td>
                        <td class="px-4 py-3 text-sm font-medium text-blue-700 whitespace-nowrap">{{ $req->total_hm ?? '00:00' }}</td>
