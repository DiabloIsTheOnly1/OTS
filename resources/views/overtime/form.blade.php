            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="position" class="w-full border p-1 rounded"
                        value="{{ old('position', $overtime->position) }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="date" class="w-full border p-1 rounded"
                        value="{{ old('date', optional($overtime->date)->format('Y-m-d') ?? now()->format('Y-m-d')) }}"
                        required>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="work_done" rows="4" class="w-full border p-1 rounded" required>{{ old('work_done', $overtime->work_done) }}</textarea>
                </div>

                <!-- Work Description -->
                <div class="md:col-span-2">
                    <label class="block font-semibold mb-1">Work to be completed during OT & _/HR</label>
                    <textarea name="reason" rows="4" class="w-full border p-1 rounded" required>{{ old('reason', $overtime->reason) }}</textarea>
                </div>
            </div>

            <!-- Submit -->
            <button class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 w-full">
                {{ $overtime->id ? 'Update Overtime Request' : 'Submit & Generate QR' }}
            </button>

            @if ($overtime->id)
                <a href="{{ route('overtime.index') }}" class="block text-center mt-3 text-blue-600 hover:underline">
                    Cancel
                </a>
            @endif
        </form>
