            ...
            <div class="bg-white shadow border rounded-lg p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <!-- EDIT MODE — dropdown with ONLY name (no reg no, no parentheses) -->
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $overtime->branch?->name ?? '-' }}" class="w-full border px-3 py-2 rounded bg-gray-100" readonly>
                    </div>

                    <!-- Department -->
                    <div>
                        <label class="block font-semibold text-gray-600 mb-1">Department</label>
                        <input type="text" value="{{ $overtime->department?->department_name ?? '-' }}" class="w-full border px-3 py-2 rounded bg-gray-100" readonly>
                    </div>
                    ...
                    <div class="md:col-span-2 bg-amber-50 border border-amber-300 rounded-lg p-5">
                        ...
                        <div class="grid grid-cols-3 gap-4 text-center">
                            <div>
                                ...
                                <p class="view-mode text-xl font-bold text-amber-800">{{ $overtime->start_time?->format('H:i') ?? '-' }}</p>
                                <input type="time" name="start_time" class="edit-mode hidden w-full text-center text-xl font-bold text-amber-800 border border-amber-400 rounded px-2 py-1"
                            ...
                            <div>
                                ...
                                <p class="view-mode text-xl font-bold text-amber-800">{{ $overtime->end_time?->format('H:i') ?? '-' }}</p>
                                <input type="time" name="end_time" class="edit-mode hidden w-full text-center text-xl font-bold text-amber-800 border border-amber-400 rounded px-2 py-1"
                        ...
                        <div class="text-center mt-4">
                            <div id="total-hours-display" class="inline-block bg-amber-700 text-white px-6 py-2 rounded-full font-bold text-lg">
                                @if($overtime->total_hours > 0)
                                    @php $h = floor($overtime->total_hours); $m = round(($overtime->total_hours - $h) * 60); @endphp
                                    {{ $h }}h {{ $m > 0 ? $m.'m' : '' }}
                                @else —
                                @endif
                            </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="reg_no" class="edit-mode hidden w-full border rounded px-3 py-2 font-mono"
