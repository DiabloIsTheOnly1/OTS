...
<div class="bg-white rounded-xl shadow-sm p-6 mb-4">
    <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4 items-end">
        ...
        <div>
            ...
            <select name="branch_id" class="mt-1 px-3 py-2 border w-full rounded-lg border-gray-300 shadow-sm">
        ...
        <div>
            ...
            <select name="department_id" class="mt-1 px-3 py-2 border w-full rounded-lg border-gray-300 shadow-sm">
                <option value="">All</option>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ request('name') }}"
                class="mt-1 px-3 py-2 border w-full rounded-lg border-gray-300 shadow-sm">
        </div>
        ...
        <div>
            ...
            <select name="status" class="mt-1 px-3 py-2 border w-full rounded-lg border-gray-300 shadow-sm">
                <option value="">All</option>
        ...
        <div class="flex gap-2 col-span-1 sm:col-span-2 md:col-span-1">
            <button type="submit"
                class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg w-full md:w-auto transition">
                Filter
            </button>

            <a href="{{ route('hr.dashboard') }}"
                class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm rounded-lg w-full md:w-auto transition text-center">
                Reset
            </a>
        </div>
    </form>
    ...
    <div class="w-full overflow-x-auto">
        <table class="w-full min-w-[900px] text-sm">
            ...
            <tbody>
                    ...
                    <tr class="{{ $bg }} border-b hover:bg-blue-50 transition hidden md:table-row">
                        ...
                        <td class="p-3">
                            @forelse ($r->clocks as $session)
                                <p class="text-xs">{{ $session->clock_in?->format('H:i') ?? '-' }} ➝ {{ $session->clock_out?->format('H:i') ?? '-' }} ({{ $session->total_hm }})</p>
                            @empty - @endforelse
                        </td>
                        ...
                        <td class="text-center p-3">
                            <span class="px-3 py-1 rounded-full text-xs font-bold
                                @if($r->status=='pending') bg-yellow-300 text-yellow-900
                                @elseif($r->status=='approved') bg-green-300 text-green-900
                            ...
                            @if ($r->status === 'pending')
                                <div class="flex gap-2 justify-center">
                                    ...
                                    <form action="{{ route('hr.overtime.reject', $r->id) }}" method="POST">@csrf
                                        <button class="bg-red-600 text-white px-3 py-1 rounded text-xs hover:bg-red-700">Reject</button>
                                    </form>
                                </div>
                            @else
                            ...
                            <div class="border rounded-lg p-4 shadow-sm space-y-3">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="px-2 py-1 rounded-full text-[10px] font-bold
                                        @if($r->status=='pending') bg-yellow-400 text-yellow-900
                                        @elseif($r->status=='approved') bg-green-400 text-green-900
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="bg-gray-50 border rounded p-2 text-xs">
                                            <div class="flex justify-between">
                                                <span>In: {{ $session->clock_in?->format('H:i') ?? '-' }}</span>
                                                <span>Out: {{ $session->clock_out?->format('H:i') ?? '-' }}</span>
                                            </div>
                                            <p class="text-blue-700 font-bold text-right mt-1">{{ $session->total_hm }}</p>
                                        </div>
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <form action="{{ route('hr.overtime.approve', $r->id) }}" method="POST" class="w-1/2">@csrf
                                            <button class="bg-green-600 text-white px-3 py-2 rounded text-xs hover:bg-green-700 w-full">Approve</button>
                                        </form>
                                        <form action="{{ route('hr.overtime.reject', $r->id) }}" method="POST" class="w-1/2">@csrf
                                            <button class="bg-red-600 text-white px-3 py-2 rounded text-xs hover:bg-red-700 w-full">Reject</button>
                                        </form>
                                    </div>
                                    @else
                                        <p class="text-[10px] uppercase font-semibold text-gray-500">Handled by</p>
                                        <p class="font-bold text-gray-800 text-xs">{{ $r->approver?->username ?? '-' }}</p>
                                    @endif
                                </div>
                ...
                @empty
                    <tr><td colspan="10" class="p-6 text-center text-sm text-gray-500">No requests found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
